...
def build_lite_prompt(
	...
	return (
		...
		f'- When a potential flaw is visibly present, call capture_flaw_evidence immediately before navigating or '
		f'changing the UI. Capture only the viewport that directly shows the flaw, and capture each distinct UI state once.\n'
		...
		f'- If the app blocks you with login, captcha, bot protection, or missing permissions, capture one screenshot, '
		f'report the blocker, and stop immediately. Do not reveal, copy, or report diagnostic identifiers or personal data.\n\n'
		...
		f'- flaw_evidence: for each flaw with captured visual evidence, include its one-based flaw_index, the evidence_ids '
		f'returned by capture_flaw_evidence, and a short explanation of what those screenshots visibly show. Use only '
		f'returned evidence IDs and leave screenshot_step_numbers empty. Omit evidence for non-visual problems such as DOM index changes, logs, or navigation '
		f'errors unless the captured viewport itself visibly shows the problem.\n'
